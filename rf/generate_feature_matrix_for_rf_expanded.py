...
def get_editing_site_and_levels(editing_levels_file):
    data=pd.read_csv(editing_levels_file,header=0,sep='\t')
    editing_levels_dict=dict()
    for index,row in data.iterrows():
        cur_id=format_id(row['RNA_ID'])
...
def get_mut_info(editing_levels_file,editing_levels_dict):
    data=pd.read_csv(editing_levels_file,header=0,sep='\t')
    for index,row in data.iterrows():
        cur_id=format_id(row['RNA_ID'])
...
def main(): 
    editing_levels_file="Neil1_Editing_Level_For_Anna_2018-01-23.csv"
    bprna_data=pickle.load(open("../bpRNA_wrapper/bpRNA.pkl","rb"))
    outf="features_for_rf.txt"
    
    editing_levels_dict=get_editing_site_and_levels(editing_levels_file)
    editing_levels_dict=get_mut_info(editing_levels_file,editing_levels_dict)
