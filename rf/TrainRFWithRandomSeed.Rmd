---
title: "Train Ensemble Model with Multiple Train/Test Splits"
...
```{r output_dir,include=FALSE}
output_dir="xin_neil1"
dir.create(paste(getwd(),output_dir,sep='/'),showWarnings = FALSE)
```
##Load Feature Matrix 
```{r joint_train,echo=FALSE, warning=FALSE,warning=FALSE}
data=read.table("xin_feature_matrices/neil1_computational.features.csv.cleaned",header=TRUE,sep='\t',row.names = 1)
...
#drop any features with 0 variance 
data=data[sapply(data,var)>0]

```

## Train and Test Splits
```{r generate_splits,echo=FALSE, warning=FALSE}
...
```{r keep_track_outputs,echo=FALSE, warning=FALSE}
...
```{r train,echo=FALSE, warning=FALSE}
...
```{r write_outputs,echo=FALSE, warning=FALSE}
...
```{r feat_importance,echo=FALSE, warning=FALSE}
...
```{r plot_features,echo=FALSE, warning=FALSE}
